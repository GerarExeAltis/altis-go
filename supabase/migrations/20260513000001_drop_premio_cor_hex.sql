-- 20260513000001_drop_premio_cor_hex.sql
-- Remove a coluna cor_hex da tabela premios. A roleta passou a alternar
-- primary/branco automaticamente entre as fatias (vide Roda.tsx), entao
-- a cor por premio nao e mais usada visualmente.
--
-- Tambem recria sortear_e_baixar_estoque() removendo cor_hex do RETURN.
-- O type que muda forca o DROP da funcao antes do CREATE OR REPLACE.

-- 1) DROP da funcao que retorna cor_hex (precisa antes do DROP COLUMN
--    porque o RETURNS TABLE referencia o tipo).
DROP FUNCTION IF EXISTS public.sortear_e_baixar_estoque(UUID);

-- 2) Remove coluna
ALTER TABLE public.premios DROP COLUMN IF EXISTS cor_hex;

-- 3) Recria sortear_e_baixar_estoque sem cor_hex
